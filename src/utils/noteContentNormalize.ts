...
import { createDefaultNoteContent, createListItem, createListItemId } from '@/utils/noteContentCreate'

export function isParagraph(block: NoteBlock): block is NoteParagraphBlock {
  return block.type === 'paragraph' && Array.isArray((block as NoteParagraphBlock).spans)
}

export function isBulletedList(block: NoteBlock): block is NoteBulletedListBlock {
...
function normalizeSpans(spans: NoteSpan[] | JsonValue | undefined): NoteSpan[] {
  if (!Array.isArray(spans) || spans.length === 0) {
    return [{ text: '' }]
  }

  return spans.map((span) => {
    const record = (typeof span === 'object' && span !== null && !Array.isArray(span)
      ? span
      : {}) as {
      text?: string
      bold?: boolean
...
export function coerceNoteBlocksFromJson(content: JsonValue | undefined): NoteBlock[] {
  if (!Array.isArray(content)) return createDefaultNoteContent()
  ...
  for (const entry of content) {
    ...
    if (record.type === 'bulleted-list' && Array.isArray(record.items)) {
      blocks.push(normalizeBulletedList({
        type: 'bulleted-list',
        items: record.items,
      }))
    }
  }
  return normalizeBlocksForApi(blocks.length ? blocks : createDefaultNoteContent())
}
...
export function normalizeBlocksForApi(content: NoteBlock[]): NoteBlock[] {
  ...
  return content.map((block) => {
    ...
    return createDefaultNoteContent()[0]
  })
}
